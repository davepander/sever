...
async function main() {
  ...
  const severBadge = await upgrades.upgradeProxy(proxy, SeverBadge, {
    kind: "uups",
  });
  severBadge.deployed();
  assert(severBadge.address === proxy, "proxy address changed");

  // Check new implementation
  const impl2 = await upgrades.erc1967.getImplementationAddress(severBadge.address);
